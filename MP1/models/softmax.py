        ...
        for i in range(N):
            ...
            for idx_class in range(C):
                if idx_class == y_train[i]:
                    ...
                    dscore = softmax[idx_class] -1
                else:
                    dscore = softmax[idx_class]

                grad_w[:,idx_class] += dscore*X_train[i]
            
            # method 2
            # for j in range(C):
            #     grad_w[:,j] += X_train[i] * softmax[j]
            # grad_w[:,y_train[i]] -= X_train[i]

            # method 3
            # dscores = softmax.reshape(1,-1)
            # dscores[:, y_train[i]] -= 1
            ...
            # grad_w += np.dot(X_train[i].T.reshape(X_train[i].shape[0], 1), dscores)
